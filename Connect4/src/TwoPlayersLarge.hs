...
module TwoPlayersLarge (twoPlayersLarge, Tick, MyT, drawUI, theMap, moveg, initGame) where
import GameLogicLarge
import Data.Maybe
import Data.List
import Prelude
...
handleEvent g (VtyEvent (V.EvKey (V.KChar '8') [])) = continue $ moveg 7 g
handleEvent g (VtyEvent (V.EvKey (V.KChar '9') [])) = continue $ moveg 8 g
...
drawBoard g = withBorderStyle BS.unicodeBold
  $ B.borderWithLabel (withAttr magBg $ str "Connect 4 Large Mode")
  $ vBox rows
  where
