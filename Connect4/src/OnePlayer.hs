...
module OnePlayer (onePlayer, Tick, MyT, drawUI, theMap, moveg, initGame) where
import GameLogicOne
import Data.Maybe
import Data.List
import Prelude
...
drawGameStatus g =
  if (not (_done g))
    then hLimit 11 $ withAttr gameOverAttr $ C.hCenter $ str ((printPlayer (_player g)) ++ "'s  move")
      ...
      _    -> hLimit 11 $ withAttr gameOverAttr $ C.hCenter $ str ((printPlayer ((_player g))) ++ " win")
...
drawBoard g = withBorderStyle BS.unicodeBold
  $ B.borderWithLabel (withAttr magBg $ str "Connect 4 AI Mode")
  $ vBox rows
  where
